...
from .models import StockDetail, CustomUser
import json
import copy
...
class StockConsumer(AsyncWebsocketConsumer):
    ...
    def addToCeleryBeat(self, stock_list):
        ...
        if len(task)>0:
            ...
            task = PeriodicTask.objects.create(interval=schedule, name='every-10-seconds', task='stockPlatform.tasks.update_stocks', args=json.dumps([stock_list]))

    @sync_to_async    
    def addToStockDetail(self, stock_list):
        user = CustomUser.objects.get(name = "custom_user")
    ...
    def selectUserStocks(self):
        user_id = CustomUser.objects.get(name = "custom_user")
        user_stocks = user_id.stockdetail_set.values_list('stock', flat = True)
